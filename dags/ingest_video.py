...
@dag(
    ...
    description="One video at a time: copy → Spark clips → VLM → fingerprint → Neo4j, then the next file in /videos.",
...
def ingest_videos():
    @task
    def list_next_video() -> list[str]:
        """Next ungraphed file in /videos, or empty if the folder is done."""
        from workers.ingest import next_new_video
    ...
    def continue_if_more(summary: dict | None) -> str:
        from workers.ingest import list_new_videos as _list

        if not summary or not summary.get("video_ids"):
            return "ingest_done"
        if _list():
            return "trigger_next_video"
        return "ingest_done"

    paths = list_next_video()
    ...
    summary = finalize(graphed)

    done = EmptyOperator(task_id="ingest_done")
    trigger = TriggerDagRunOperator(
        task_id="trigger_next_video",
        trigger_dag_id="ingest_videos",
        wait_for_completion=False,
        reset_dag_run=False,
    )
    continue_if_more(summary) >> [trigger, done]
